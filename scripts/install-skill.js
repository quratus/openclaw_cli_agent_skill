#!/usr/bin/env node
/**
 * Register the kimi-worker skill so OpenClaw agents can see it.
 * Symlinks this repo's skills/kimi-worker into ~/.openclaw/skills/kimi-worker
...
const skillSource = path.join(repoRoot, "skills", "kimi-worker");
const skillsDir =
  process.env.OPENCLAW_SKILLS_DIR ||
  path.join(process.env.HOME || process.env.USERPROFILE || "", ".openclaw", "skills");
const skillTarget = path.join(skillsDir, "kimi-worker");
...
try {
  ...
  console.log("Skill registered: kimi-worker ->", skillTarget);
